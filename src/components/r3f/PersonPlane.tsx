...
interface PersonPlaneProps {
  ...
  calibratedRef?: MutableRefObject<{
    x: number;
    y: number;
    total?: number;
  } | null>;
...
export default function PersonPlane({
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      if (
        (calibratedRef && calibratedRef.current) ||
        (copRef && copRef.current)
      ) {
        const cop =
          calibratedRef && calibratedRef.current
        ...
        // ボードは波の傾きに常に追従させる（cop.x との差分ではなく波そのものを使う）
        // バランス判定は GameScene のゲームループ側で別途行う
        const amplifiedRoll =
          Math.sign(waveTarget) * Math.pow(Math.abs(waveTarget), 0.9) * 1.8;
      ...
      {enableSurfboard &&
        ...
        (gltf && gltf.scene ? (
          ...
          <group
            ...
            position={[
              0,
              -PLANE_HEIGHT / 2 - SURFBOARD_THICKNESS / 2 - surfboardGap + 0.7,
              0.5,
            ]}
          ...
          >
            <boxGeometry
              args={[SURFBOARD_LENGTH, SURFBOARD_THICKNESS, SURFBOARD_WIDTH]}
            />
            <meshStandardMaterial
              color={surfboardColor}
              metalness={0.2}
              roughness={0.6}
            />
          </mesh>
